...
pub const Codegen = struct {
    ...
    pub fn generate(self: *Self, options: CodegenOptions) ![]const u8 {
        ...
        try self.emitTermEntrypoints(options);
    ...
    fn emitTermEntrypoints(self: *Self, options: CodegenOptions) !void {
        const writer = self.output.writer(self.allocator);
        for (self.termenv.terms.items) |term| {
            ...
            const ret_prefix = if (decl.partial) "!?" else "!";
            try writer.writeAll(") ");
            try writer.writeAll(ret_prefix);
