...
test "RegClass: count is correct" {
    try testing.expectEqual(@as(u8, 5), RegClass.count);
}
...
test "RegClass: index returns correct values" {
    ...
    try testing.expectEqual(@as(u8, 3), RegClass.scalable_vector.index());
    try testing.expectEqual(@as(u8, 4), RegClass.predicate.index());
}
...
test "RegClass: all classes have unique indices" {
    ...
    try testing.expect(RegClass.vector.index() != RegClass.scalable_vector.index());
    try testing.expect(RegClass.scalable_vector.index() != RegClass.predicate.index());
}
...
test "PReg.new: all register classes" {
    ...
    const sve_reg = PReg.new(.scalable_vector, 5);
    const pred_reg = PReg.new(.predicate, 7);
    ...
    try testing.expectEqual(RegClass.scalable_vector, sve_reg.class());
    try testing.expectEqual(RegClass.predicate, pred_reg.class());
    ...
    try testing.expectEqual(@as(u6, 5), sve_reg.hwEnc());
    try testing.expectEqual(@as(u6, 7), pred_reg.hwEnc());
}
...
test "PReg.new: encoding roundtrip" {
    const test_cases = [_]struct { class: RegClass, hw: u6 }{
        ...
        .{ .class = .vector, .hw = 0 },
        .{ .class = .vector, .hw = 31 },
        .{ .class = .scalable_vector, .hw = 31 },
        .{ .class = .predicate, .hw = 15 },
    };
...
test "PReg: max hw_enc is 31 (5 bits)" {
    const preg = PReg.new(.int, 31);
    try testing.expectEqual(@as(u6, 31), preg.hwEnc());
}
...
test "VReg.new: all register classes" {
    ...
    const sve_vreg = VReg.new(400, .scalable_vector);
    const pred_vreg = VReg.new(500, .predicate);
    ...
    try testing.expectEqual(RegClass.scalable_vector, sve_vreg.class());
    try testing.expectEqual(RegClass.predicate, pred_vreg.class());
    ...
    try testing.expectEqual(@as(u32, 400), sve_vreg.index());
    try testing.expectEqual(@as(u32, 500), pred_vreg.index());
}
...
test "VReg.new: encoding roundtrip" {
    const test_cases = [_]struct { idx: u32, class: RegClass }{
        ...
        .{ .idx = 0x1FFF_FFFF, .class = .int }, // Max index (29 bits)
        .{ .idx = 0x1FFF_FFFF, .class = .predicate },
    };
...
test "VReg: max index is 0x1FFFFFFF (29 bits)" {
    const vreg = VReg.new(0x1FFF_FFFF, .int);
    try testing.expectEqual(@as(u32, 0x1FFF_FFFF), vreg.index());
}
...
test "Reg.fromPReg roundtrip preserves class" {
    const classes = [_]RegClass{ .int, .float, .vector, .scalable_vector, .predicate };
    for (classes) |rc| {
...
test "RegClass: enum values are sequential" {
    ...
    try testing.expectEqual(@as(u8, 3), @intFromEnum(RegClass.scalable_vector));
    try testing.expectEqual(@as(u8, 4), @intFromEnum(RegClass.predicate));
}
