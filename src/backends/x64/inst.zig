...
pub const Inst = union(enum) {
    ...
    pub fn format(
        ...
        switch (self) {
            .mov_rr => |i| try writer.print("mov.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .mov_imm => |i| try writer.print("mov.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .add_rr => |i| try writer.print("add.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .add_imm => |i| try writer.print("add.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .sub_rr => |i| try writer.print("sub.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .sub_imm => |i| try writer.print("sub.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .and_rr => |i| try writer.print("and.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .and_imm => |i| try writer.print("and.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .or_rr => |i| try writer.print("or.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .or_imm => |i| try writer.print("or.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .xor_rr => |i| try writer.print("xor.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .xor_imm => |i| try writer.print("xor.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .cmp_rr => |i| try writer.print("cmp.{any} {any}, {any}", .{ i.size, i.lhs, i.rhs }),
            .cmp_imm => |i| try writer.print("cmp.{any} {any}, ${d}", .{ i.size, i.lhs, i.imm }),
            .test_rr => |i| try writer.print("test.{any} {any}, {any}", .{ i.size, i.lhs, i.rhs }),
            .test_imm => |i| try writer.print("test.{any} {any}, ${d}", .{ i.size, i.lhs, i.imm }),
            .shl_rr => |i| try writer.print("shl.{any} {any}, {any}", .{ i.size, i.dst, i.count }),
            .shl_imm => |i| try writer.print("shl.{any} {any}, ${d}", .{ i.size, i.dst, i.count }),
            .shr_rr => |i| try writer.print("shr.{any} {any}, {any}", .{ i.size, i.dst, i.count }),
            .shr_imm => |i| try writer.print("shr.{any} {any}, ${d}", .{ i.size, i.dst, i.count }),
            .sar_rr => |i| try writer.print("sar.{any} {any}, {any}", .{ i.size, i.dst, i.count }),
            .sar_imm => |i| try writer.print("sar.{any} {any}, ${d}", .{ i.size, i.dst, i.count }),
            .rol_rr => |i| try writer.print("rol.{any} {any}, {any}", .{ i.size, i.dst, i.count }),
            .rol_imm => |i| try writer.print("rol.{any} {any}, ${d}", .{ i.size, i.dst, i.count }),
            .ror_rr => |i| try writer.print("ror.{any} {any}, {any}", .{ i.size, i.dst, i.count }),
            .ror_imm => |i| try writer.print("ror.{any} {any}, ${d}", .{ i.size, i.dst, i.count }),
            .imul_rr => |i| try writer.print("imul.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .imul_imm => |i| try writer.print("imul.{any} {any}, {any}, ${d}", .{ i.size, i.dst, i.src, i.imm }),
            .neg => |i| try writer.print("neg.{any} {any}", .{ i.size, i.dst }),
            .not => |i| try writer.print("not.{any} {any}", .{ i.size, i.dst }),
            .push_r => |i| try writer.print("push {any}", .{i.src}),
            .pop_r => |i| try writer.print("pop {any}", .{i.dst}),
            .load => |i| try writer.print("load.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .store => |i| try writer.print("store.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .lea => |i| try writer.print("lea {any}, {any}", .{ i.dst, i.src }),
            .mov_rm => |i| try writer.print("mov.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .mov_mr => |i| try writer.print("mov.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .mov_mi => |i| try writer.print("mov.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .jmp => |i| try writer.print("jmp {any}", .{i.target}),
            .jmp_cond => |i| try writer.print("j{any} {any}", .{ i.cc, i.target }),
            .call => |i| try writer.print("call {any}", .{i.target}),
            .setcc => |i| try writer.print("set{any} {any}", .{ i.cc, i.dst }),
            .cmov => |i| try writer.print("cmov{any}.{any} {any}, {any}", .{ i.cc, i.size, i.dst, i.src }),
            .ret => try writer.writeAll("ret"),
            .ret_imm => |i| try writer.print("ret ${d}", .{i.imm}),
            .nop => try writer.writeAll("nop"),
            .movdqa_rr => |i| try writer.print("movdqa {any}, {any}", .{ i.dst, i.src }),
            .movdqa_mr => |i| try writer.print("movdqa {any}, {any}", .{ i.dst, i.src }),
            .movdqa_rm => |i| try writer.print("movdqa {any}, {any}", .{ i.dst, i.src }),
            .movdqu_rr => |i| try writer.print("movdqu {any}, {any}", .{ i.dst, i.src }),
            .movdqu_mr => |i| try writer.print("movdqu {any}, {any}", .{ i.dst, i.src }),
            .movdqu_rm => |i| try writer.print("movdqu {any}, {any}", .{ i.dst, i.src }),
            .movups_rr => |i| try writer.print("movups {any}, {any}", .{ i.dst, i.src }),
            .movups_mr => |i| try writer.print("movups {any}, {any}", .{ i.dst, i.src }),
            .movups_rm => |i| try writer.print("movups {any}, {any}", .{ i.dst, i.src }),
            .movupd_rr => |i| try writer.print("movupd {any}, {any}", .{ i.dst, i.src }),
            .movupd_mr => |i| try writer.print("movupd {any}, {any}", .{ i.dst, i.src }),
            .movupd_rm => |i| try writer.print("movupd {any}, {any}", .{ i.dst, i.src }),
            .movss_rr => |i| try writer.print("movss {any}, {any}", .{ i.dst, i.src }),
            .movss_mr => |i| try writer.print("movss {any}, {any}", .{ i.dst, i.src }),
            .movss_rm => |i| try writer.print("movss {any}, {any}", .{ i.dst, i.src }),
            .movsd_rr => |i| try writer.print("movsd {any}, {any}", .{ i.dst, i.src }),
            .movsd_mr => |i| try writer.print("movsd {any}, {any}", .{ i.dst, i.src }),
            .movsd_rm => |i| try writer.print("movsd {any}, {any}", .{ i.dst, i.src }),
            .paddd_rr => |i| try writer.print("paddd {any}, {any}", .{ i.dst, i.src }),
            .paddq_rr => |i| try writer.print("paddq {any}, {any}", .{ i.dst, i.src }),
            .psubd_rr => |i| try writer.print("psubd {any}, {any}", .{ i.dst, i.src }),
            .psubq_rr => |i| try writer.print("psubq {any}, {any}", .{ i.dst, i.src }),
            .pmulld_rr => |i| try writer.print("pmulld {any}, {any}", .{ i.dst, i.src }),
            .pand_rr => |i| try writer.print("pand {any}, {any}", .{ i.dst, i.src }),
            .por_rr => |i| try writer.print("por {any}, {any}", .{ i.dst, i.src }),
            .pxor_rr => |i| try writer.print("pxor {any}, {any}", .{ i.dst, i.src }),
            .pandn_rr => |i| try writer.print("pandn {any}, {any}", .{ i.dst, i.src }),
            .pcmpeqd_rr => |i| try writer.print("pcmpeqd {any}, {any}", .{ i.dst, i.src }),
            .pcmpeqq_rr => |i| try writer.print("pcmpeqq {any}, {any}", .{ i.dst, i.src }),
            .pcmpgtd_rr => |i| try writer.print("pcmpgtd {any}, {any}", .{ i.dst, i.src }),
            .pcmpgtq_rr => |i| try writer.print("pcmpgtq {any}, {any}", .{ i.dst, i.src }),
            .pslld_imm => |i| try writer.print("pslld {any}, ${d}", .{ i.dst, i.count }),
            .psrld_imm => |i| try writer.print("psrld {any}, ${d}", .{ i.dst, i.count }),
            .psrad_imm => |i| try writer.print("psrad {any}, ${d}", .{ i.dst, i.count }),
            .psllq_imm => |i| try writer.print("psllq {any}, ${d}", .{ i.dst, i.count }),
            .psrlq_imm => |i| try writer.print("psrlq {any}, ${d}", .{ i.dst, i.count }),
            .shufps_rr => |i| try writer.print("shufps {any}, {any}, ${d}", .{ i.dst, i.src, i.imm }),
            .shufpd_rr => |i| try writer.print("shufpd {any}, {any}, ${d}", .{ i.dst, i.src, i.imm }),
            .pshufd_rr => |i| try writer.print("pshufd {any}, {any}, ${d}", .{ i.dst, i.src, i.imm }),
            .addps_rr => |i| try writer.print("addps {any}, {any}", .{ i.dst, i.src }),
            .addpd_rr => |i| try writer.print("addpd {any}, {any}", .{ i.dst, i.src }),
            .subps_rr => |i| try writer.print("subps {any}, {any}", .{ i.dst, i.src }),
            .subpd_rr => |i| try writer.print("subpd {any}, {any}", .{ i.dst, i.src }),
            .mulps_rr => |i| try writer.print("mulps {any}, {any}", .{ i.dst, i.src }),
            .mulpd_rr => |i| try writer.print("mulpd {any}, {any}", .{ i.dst, i.src }),
            .divps_rr => |i| try writer.print("divps {any}, {any}", .{ i.dst, i.src }),
            .divpd_rr => |i| try writer.print("divpd {any}, {any}", .{ i.dst, i.src }),
            .addss_rr => |i| try writer.print("addss {any}, {any}", .{ i.dst, i.src }),
            .addsd_rr => |i| try writer.print("addsd {any}, {any}", .{ i.dst, i.src }),
            .subss_rr => |i| try writer.print("subss {any}, {any}", .{ i.dst, i.src }),
            .subsd_rr => |i| try writer.print("subsd {any}, {any}", .{ i.dst, i.src }),
            .mulss_rr => |i| try writer.print("mulss {any}, {any}", .{ i.dst, i.src }),
            .mulsd_rr => |i| try writer.print("mulsd {any}, {any}", .{ i.dst, i.src }),
            .divss_rr => |i| try writer.print("divss {any}, {any}", .{ i.dst, i.src }),
            .divsd_rr => |i| try writer.print("divsd {any}, {any}", .{ i.dst, i.src }),
            .sqrtss_rr => |i| try writer.print("sqrtss {any}, {any}", .{ i.dst, i.src }),
            .sqrtsd_rr => |i| try writer.print("sqrtsd {any}, {any}", .{ i.dst, i.src }),
            .minss_rr => |i| try writer.print("minss {any}, {any}", .{ i.dst, i.src }),
            .minsd_rr => |i| try writer.print("minsd {any}, {any}", .{ i.dst, i.src }),
            .maxss_rr => |i| try writer.print("maxss {any}, {any}", .{ i.dst, i.src }),
            .maxsd_rr => |i| try writer.print("maxsd {any}, {any}", .{ i.dst, i.src }),
            .cmpxchg_mr => |i| try writer.print("lock cmpxchg.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .xadd_mr => |i| try writer.print("lock xadd.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .xchg_mr => |i| try writer.print("lock xchg.{any} {any}, {any}", .{ i.size, i.dst, i.src }),
            .lock_add_mi => |i| try writer.print("lock add.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .lock_sub_mi => |i| try writer.print("lock sub.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .lock_and_mi => |i| try writer.print("lock and.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .lock_or_mi => |i| try writer.print("lock or.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
            .lock_xor_mi => |i| try writer.print("lock xor.{any} {any}, ${d}", .{ i.size, i.dst, i.imm }),
...
pub const CallTarget = union(enum) {
    ...
    pub fn format(
        ...
        switch (self) {
            ...
            .indirect => |reg| try writer.print("*{any}", .{reg}),
        }
    }
};
...
test "ALU instruction formatting" {
    ...
    const str1 = try std.fmt.bufPrint(&buf, "{any}", .{add_imm});
...
test "setcc and cmov formatting" {
    ...
    const str1 = try std.fmt.bufPrint(&buf, "{any}", .{setcc});
    try testing.expect(std.mem.indexOf(u8, str1, "setcc") != null);
    try testing.expect(std.mem.indexOf(u8, str1, "e") != null);

    const cmov = Inst{ .cmov = .{ .cc = .l, .dst = wr0, .src = r1, .size = .size64 } };
    const str2 = try std.fmt.bufPrint(&buf, "{any}", .{cmov});
    try testing.expect(std.mem.indexOf(u8, str2, "cmov") != null);
    try testing.expect(std.mem.indexOf(u8, str2, "l") != null);
}

test "ret_imm formatting" {
    var buf: [512]u8 = undefined;
    const ret_imm = Inst{ .ret_imm = .{ .imm = 8 } };
    const str = try std.fmt.bufPrint(&buf, "{any}", .{ret_imm});
