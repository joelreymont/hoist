...
pub const Aarch64ABICallee = struct {
    ...
    pub fn emitPrologue(
        ...
        // STP X29, X30, [SP, #-frame_size]!  (pre-index)
        ...
        // Calculate total frame size (FP+LR + callee-saves + locals)
        const frame_offset: i16 = -16; // FP+LR = 16 bytes
        ...
        for (self.clobbered_callee_saves.items) |preg| {
            // Use STR for individual saves (STP for pairs would be better)
            const reg = Reg.fromPReg(preg);
            try emit_fn(.{
                .str = .{
                    .src = reg,
                    .base = sp,
                    .offset = -8, // Simplified offset
                    .size = .size64,
                },
            }, buffer);
        }
    ...
    /// Restores callee-saves, FP, LR, and returns.
    pub fn emitEpilogue(
        self: *Aarch64ABICallee,
        buffer: *buffer_mod.MachBuffer,
    ) !void {
